...
public class Boss : MonoBehaviour
{
    ...
    void Update()
    {
        if (wakeRange > Math.Abs(Math.Sqrt(Math.Pow(playerObj.transform.position.x, 2) + Math.Pow(playerObj.transform.position.y, 2)) -
            Math.Sqrt(Math.Pow(this.transform.position.x, 2) + Math.Pow(this.transform.position.y, 2))))
        {
            wake = true;
        }
